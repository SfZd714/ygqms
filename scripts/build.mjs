...
for(const f of files){let raw=fs.readFileSync(path.join(src,f),'utf8');let title=titleOf(f);let body=marked.parse(clean(raw.replace(/^---[\s\S]*?---/,'').trim()));fs.writeFileSync(path.join(out,fileName(f)),page(title,body));}
const indexBody='<h1>央国企 / 科研院所面试知识库</h1><p>项目表达、技术总结、结构化面试和研究所求职方法。</p><h2>页面目录</h2><ul>'+files.map(f=>`<li><a href="${slug(f)}.html">${f.replace(/\.md$/,'')}</a></li>`).join('')+'</ul>';fs.writeFileSync(path.join(out,'index.html'),page('首页',indexBody));
