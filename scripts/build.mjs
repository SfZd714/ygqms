...
const categoryCards=categoryOrder.map((cat,catIndex)=>{const items=docs.filter(d=>d.category===cat);return `<section id="${slug(cat)}" class="category-card cat-${catIndex+1}"><div class="category-card-head"><span class="category-icon">${categoryIcons[cat]}</span><h3>${categoryLabels[cat]}</h3><span class="count-badge">${items.length} 篇</span></div><p>${categoryDescriptions[cat]}</p><div class="doc-list">${items.map((d,i)=>`<a class="doc-link" href="${docHref(d.title)}"><span class="doc-index">${String(i+1).padStart(2,'0')}</span><span class="doc-title">${esc(d.title)}</span><span class="doc-arrow">→</span></a>`).join('')}</div></section>`}).join('');
